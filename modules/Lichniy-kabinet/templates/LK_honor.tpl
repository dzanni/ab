    ...
    <table class="table_lk table__add">
        ...
        <tr class="js_add_new_element">
            {foreach from=$fields item=f}
                {if $f.chg && !$f.notShow}
                    {if !$f.v}
                    <td>{if $f.search}<input type="text" class="js_add slyle_input" data-field="f_{$f.field}" /> {/if}</td>
                    {else}
            <td>
                <select class="js_add select" data-field="f_{$f.field}">
            ...
            <tr class="js_search_block">
                {foreach from=$fields item=f}
                    ...
                    <th>
                        {if $f.search}
                            {if $f.v}
                                    <select class="js_search select" data-field="s_{$f.field}">
                                        <option value="">Выберите значение</option>
                                        {foreach from=$f.v item=val}
                                            <option value="{$val.id}">{$val.title}</option>
                                        {/foreach}
