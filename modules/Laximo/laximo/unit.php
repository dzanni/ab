...
class DetailExtender extends CommonExtender
{
	...
	{
        if ($dataItem['filter'])
            $link = "?".'sr=1&type=detailfilter&c=' . $catalog . '&vid=' . $_GET['vid'] . '&uid=' . $_GET['uid'] .  '&cid=' . $_GET['cid'] . 'did=' . $dataItem['detailid'] . '&ssd=' . $dataItem['ssd'] . '&f=' . urlencode($dataItem['filter']);
        else {
...
else
{
	...
	$renderer->detaillistrenderer->columns = array('Toggle'=>1, 'PNC'=>3, 'OEM'=>2, 'Name'=>3,'Note'=>2, 'Tooltip'=>1);
