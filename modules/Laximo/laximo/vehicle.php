...
class CategoryExtender extends CommonExtender
{
    function FormatLink($type, $dataItem, $catalog, $renderer)
    {
        ...
        if ($type == 'quickgroup')
			$link = "?".'sr=1&type=qgroups&c='. $catalog . '&vid=' . $renderer->vehicleid . '&ssd=' . $renderer->ssd;
        else
            $link = "?".'sr=1&type=vehicle&c=' . $catalog . '&vid=' . $renderer->vehicleid . '&cid=' . $dataItem['categoryid'] . '&ssd=' . ($ssd ? $ssd : $renderer->ssd);
...
class UnitExtender extends CommonExtender
{
    ...
    {
        if ($type == 'filter')
            $link = "?".'sr=1&type=unitfilter&c=' . $catalog . '&vid=' . $renderer->vehicle_id . '&uid=' . $dataItem['unitid'] .  '&cid=' . $renderer->categoryid . '&ssd=' . $dataItem['ssd'] . '&f=' . urlencode($dataItem['filter']);
        else
		 $link = "?".'sr=1&type=unit&c=' . $catalog . '&vid=' . $renderer->vehicle_id . '&uid=' . $dataItem['unitid'] . '&cid=' . $renderer->categoryid . '&ssd=' . $dataItem['ssd'];
...
if ($request->error != '') {
    ...
    if (array_key_exists('checkQG', $_GET) && CommonExtender::isFeatureSupported($catalogInfo, 'quickgroups')) {
        $link = "?".'sr=1&type=qgroups&c=' . $_GET['c'] . '&vid=' . $_GET['vid'] . '&ssd=' . $_GET['ssd'] . '&path_data=' . $_GET['path_data'];
