    ...
    function FormatLink($type, $dataItem, $catalog, $renderer)
    {
        if (!$catalog)
            $catalog = $dataItem['catalog'];
       		$link = "?"."sr=1&type=".($renderer->qg == 1 ? 'qgroups' : 'vehicle') . '&c=' . $catalog . '&vid=' . $dataItem['vehicleid'] . '&ssd=' . $dataItem['ssd'] . ($renderer->qg == -1 ? '&checkQG': ''). '&path_data=' . urlencode(base64_encode(substr($dataItem['vehicle_info'], 0, 300)));
