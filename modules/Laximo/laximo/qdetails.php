...
class QuickDetailsExtender extends CommonExtender
{
    function FormatLink($type, $dataItem, $catalog, $renderer)
    {
        ...
        elseif ($type == 'category')
            $link = "?".'sr=1&type=vehicle&c=' . $catalog . '&vid=' . $renderer->vehicleid . '&cid=' . $dataItem['categoryid'] . '&ssd=' . $dataItem['ssd'];
        elseif ($type == 'unit')
        {
            $coi = array();
			...
			$link = "?".'sr=1&type=unit&c=' . $catalog . '&vid=' . $renderer->vehicleid . '&uid=' . $dataItem['unitid'] .  '&cid=' . $renderer->currentunit['categoryid'] . '&ssd=' . $dataItem['ssd'] . '&coi=' . implode(',', $coi);
        }
        elseif ($type == 'detail') {
